component extends="org.lucee.cfml.test.LuceeTestCase" labels="s3" {
	...
	private function testit(cred) {
		try {
			...
			var srcBucketName=cred.PREFIX&"src-move"&listFirst(replace(server.lucee.version,".","","all"),"-")&"-"&createUniqueId();
			var trgBucketName=cred.PREFIX&"trg-move"&listFirst(replace(server.lucee.version,".","","all"),"-")&"-"&createUniqueId();
