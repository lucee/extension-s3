...
public class AccessControlListUtil {
	...
	public static GrantAndPermission[] toGrantAndPermissions(Object obj) throws S3Exception, PageException {
		CFMLEngine engine = CFMLEngineFactory.getInstance();
		if(engine.getDecisionUtil().isArray(obj)) 
			return toGrantAndPermissions(engine.getCastUtil().toArray(obj));
	...
	public static Permission toPermission(Struct sct) throws S3Exception {
		...
		if(engine.getStringUtil().isEmpty(permission,true))
		...
		if("FULLCONTROL".equals(permission))
			...
			throw new S3Exception("invalid permission definition ["+p+"], valid permissions are [FULL_CONTROL, WRITE, WRITE_ACP, READ, READ_ACP]");
	}
	...
	private static GroupGrantee toGroupGrantee(Struct sct) throws S3Exception {
		...
		if(oGroup!=null) {
			...
			if("all".equalsIgnoreCase(group) || "allusers".equalsIgnoreCase(group))
			...
			throw new S3Exception("invalid group definition ["+group+"], valid group defintions are are "
					+ "[all,authenticated,log_delivery]");
		}
		return null;
	}
	...
	private static Struct toStruct(GranteeInterface grantee) {
		...
		if(grantee instanceof GroupGrantee) {
			if(GroupGrantee.ALL_USERS.equals(grantee)) 
				sct.setEL(creator.createKey("group"), "all");
			else if(GroupGrantee.AUTHENTICATED_USERS.equals(grantee)) 
				sct.setEL(creator.createKey("group"), "authenticated");
			else if(GroupGrantee.LOG_DELIVERY.equals(grantee)) 
				sct.setEL(creator.createKey("group"), "log_delivery");
		}
