...
public class TestCase {
	...
	public static void main(String[] args) throws IOException, ServiceException {
		...
		// S3
			...
			if(!s3.isFile(bucketName, subsub))
				s3.write(bucketName, subsub, "hello text", null, null, null, null);
	...
	private static void contains(List<S3Info> buckets, String bucketName,String objectName) {
		...
		while(it.hasNext()) {
			S3Info i = it.next();
			if(
					(bucketName==null || bucketName.equals(i.getBucketName())) &&
					(objectName==null || objectName.equals(i.getObjectName()))) return;
	...
	private static void dump(String label, List<S3Info> infos) {
		System.err.println("-----------------------------------");
		System.err.println("------------ "+label+" ------------");
		System.err.println("-----------------------------------");
		Iterator<S3Info> it = infos.iterator();
		while(it.hasNext()) {
			S3Info i = it.next();
			System.err.println("= "+i.getBucketName()+":"+i.getObjectName());
		}
	}
