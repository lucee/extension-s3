...
public class MultipartUtil {
	private static final Log log = LogFactory.getLog(MultipartUtil.class);

	/**
	 * Minimum multipart upload part size supported by S3: 5 MB. NOTE: This minimum size does not apply
	 * to the last part in a multipart upload, which may be 1 byte or larger.
	 */
	...
	/**
	 * @param maxPartSize the maximum size of objects that will be generated or upload by this instance,
	 *            must be between {@link #MIN_PART_SIZE} and {@link #MAX_OBJECT_SIZE}.
	 */
	public MultipartUtil(long maxPartSize) {
		if (maxPartSize < MIN_PART_SIZE) {
			throw new IllegalArgumentException("Maximum part size parameter " + maxPartSize + " is less than the minimum legal part size " + MIN_PART_SIZE);
		}
		if (maxPartSize > MAX_OBJECT_SIZE) {
			throw new IllegalArgumentException("Maximum part size parameter " + maxPartSize + " is greater than the maximum legal upload object size " + MAX_OBJECT_SIZE);
		}
	...
	/**
	 * @return maximum part size as set in constructor.
	 */
	...
	/**
	 * @param file
	 * @return true if the given file is larger than the maximum part size defined in this instances.
	 */
	...
	/**
	 * Split the given file into objects such that no object has a size greater than the defined maximum
	 * part size. Each object uses a {@link SegmentedRepeatableFileInputStream} input stream to manage
	 * its own byte range within the underlying file.
	 *
	 * @param objectKey the object key name to apply to all objects returned by this method.
	 * @param file a file to split into multiple parts.
	 * @return an ordered list of objects that can be uploaded as multipart parts to S3 to re-constitute
	 *         the given file in the service.
	 *
	 * @throws IOException
	...
	public List<S3Object> splitFileIntoObjectsByMaxPartSize(String objectKey, File file) throws IOException, NoSuchAlgorithmException {
		...
		if (log.isDebugEnabled()) {
			log.debug("Splitting file " + file.getAbsolutePath() + " of " + fileLength + " bytes into " + partCount + " object parts with a maximum part size of " + maxPartSize);
		}
		...
		for (long offset = 0; offset < partCount; offset++) {
			...
			if (offset < partCount - 1) {
				...
				segFIS = new SegmentedRepeatableFileInputStream(file, offset * maxPartSize, maxPartSize);
			}
			else {
				...
				segFIS = new SegmentedRepeatableFileInputStream(file, offset * maxPartSize, partLength);
			}
	...
	/**
	 * Upload one or more file-based objects to S3 as multipart uploads, where each object's underlying
	 * file is split into parts based on the value of {@link #maxPartSize}.
	 *
	 * Objects are uploaded in parallel using a {@link ThreadedS3Service} class that is created within
	 * this method, so uploads will take place using as many connections and threads as are configured
	 * in your service's {@link Jets3tProperties}.
	 *
	 * This method can upload small files that don't need to be split into parts, but because there is
	 * extra overhead in performing unnecessary multipart upload operations you should avoid doing so
	 * unless it's really necessary.
	 *
	 * @param bucketName the target bucket name
	 * @param s3Service the S3 service that will perform the work
	 * @param objectsForMultipartUpload a list of one or more objects that will be uploaded, potentially
	 *            in multiple parts if the object's underlying file is larger than {@link #maxPartSize}
	 * @param eventListener an event listener to monitor progress event notifications, which should
	 *            recognize and handle error events. May be null, in which case a standard
	 *            {@link S3ServiceEventAdaptor} is used which won't report on events but will throw an
	...
	public void uploadObjects(String bucketName, S3Service s3Service, List<StorageObject> objectsForMultipartUpload, S3ServiceEventListener eventListener) throws Exception {
		...
		StorageServiceEventAdaptor captureMultipartUploadObjectsEventAdaptor = new S3ServiceEventAdaptor() {
			@Override
		...
		try {
			ThreadedS3Service threadedS3Service = new ThreadedS3Service(s3Service, eventListener);
			...
			final Map<String, StorageObject> objectsByKey = new HashMap<String, StorageObject>();
			for (StorageObject object: objectsForMultipartUpload) {
				if (object.getDataInputFile() == null) {
					throw new ServiceException(
							"MultipartUtils#uploadObjects only supports file-based" + " storage objects with a non-null getDataInputFile" + " value. Invalid object: " + object);
				}
			...
			for (MultipartUpload upload: multipartUploadList) {
				...
				partObjects = splitFileIntoObjectsByMaxPartSize(upload.getObjectKey(), object.getDataInputFile());
