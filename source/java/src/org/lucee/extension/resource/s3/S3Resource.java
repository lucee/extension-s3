...
public final class S3Resource extends ResourceSupport {
	...
	private S3Resource(CFMLEngine engine,S3 s3,int storage, S3ResourceProvider provider, String buckedName,String objectName, boolean newPattern) {
		super(engine);
		this.s3=s3;
	...
	S3Resource(CFMLEngine engine,S3 s3,int storage, S3ResourceProvider provider, String path, boolean newPattern) {
		super(engine);
		this.s3=s3;
		this.provider=provider;
		this.newPattern=newPattern;
	...
	public void createDirectory(boolean createParentWhenNotExists) throws IOException {
		...
		try {
			...
			else s3.put(bucketName, objectName+"/", acl,"<empty>",null,null);	
		}
	...
	public void createFile(boolean createParentWhenNotExists) throws IOException {
		...
		try {
			provider.lock(this);
			s3.put(bucketName, objectName, acl,"<empty>",null,null);// TODO Lucee reconize this <empty>
		} 
		catch (Exception e) {
			throw new IOException(e.getMessage());
		}
	...
	private String getPrefix()  {
		...
		if(!engine.getStringUtil().isEmpty(aki)){
			sb.append(aki);
	...
	public Resource getParentResource() {
		if(isRoot()) return null;
		return new S3Resource(engine,s3,isBucket()?S3.STORAGE_UNKNOW:storage,provider,getInnerParent(),newPattern);// MUST direkter machen
	}
	...
	public OutputStream getOutputStream(boolean append) throws IOException {
		...
		try {
			...
			if(append && !(barr==null || barr.length==0))
				engine.getIOUtil().copy(new ByteArrayInputStream(barr),os,true,true);
			return os;
		}
		catch(IOException e) {
			throw e;
	...
	public Resource getRealResource(String realpath) {
		...
		return new S3Resource(engine,s3,S3.STORAGE_UNKNOW,provider,realpath,newPattern);
	}
							...
							if(bucketExists){
								...
								if(contents.length>0) {
									...
									for(int i=0;i<contents.length;i++) {
										if(engine.getResourceUtil().translatePath(contents[i].getKey(),false,false).equals(path)) {
											has=true;
											info=contents[i];
											infoLastAccess=System.currentTimeMillis()+provider.getCache();
											break;
									...
									if(!has){
										for(int i=0;i<contents.length;i++) {
											if(engine.getResourceUtil().translatePath(contents[i].getKey(),false,false).startsWith(path)) {
		...
		try {
			if(isRoot()) {
				...
				for(int i=0;i<children.length;i++) {
					children[i]=new S3Resource(engine,s3,storage,provider,buckets[i].getName(),"",newPattern);
				...
				for(int i=0;i<contents.length;i++) {
					key=engine.getResourceUtil().translatePath(contents[i].getKey(), false, false);
					...
					if(path==null){
						names.add(name);
						tmp.add(r=new S3Resource(engine,s3,storage,provider,contents[i].getBucketName(),key,newPattern));
				...
				while(it.hasNext()) {
					...
					tmp.add(r=new S3Resource(engine,s3,storage,provider,bucketName,path,newPattern));
