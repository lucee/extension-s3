...
public class S3SetMetaDatax extends S3Function {

	private static final long serialVersionUID = 6627533385114703836L;

	@Override
	public Object invoke(PageContext pc, Object[] args) throws PageException {
		CFMLEngine eng = CFMLEngineFactory.getInstance();
		Cast cast = eng.getCastUtil();
		if (args.length > 7 || args.length < 3) throw eng.getExceptionUtil().createFunctionException(pc, "S3SetMetaData", 3, 7, args.length);

		// required
		String bucketName = cast.toString(args[0]);
		String objectName = cast.toString(args[1]);
		Struct metadata = cast.toStruct(args[2]);
